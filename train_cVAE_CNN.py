from models.cVAE_CNN import Encoder, Decoder 
...
for ep in range(N_EPOCHS):
    for x, c in train_iter: 
        ...
        # print(x_out.reshape(len(x), 28*28).shape)
        # print(x.reshape(len(x), 28*28).shape)
        # print(mu_z.shape)
        # print(log_var_z.shape)
        # exit()
        # Loss 
        loss = loss_fn(x_out.reshape(N, 28*28), x.reshape(N, 28*28), mu_z, log_var_z)
        # Backward
        loss.backward()
    ...
    with torch.no_grad():
        ...
        # Loss 
        loss = loss_fn(x_out.reshape(N, 28*28), X_train.reshape(N, 28*28), mu_z, log_var_z)
        ...
        # Loss 
        loss = loss_fn(x_out.reshape(N, 28*28), X_test.reshape(N, 28*28), mu_z, log_var_z)
...
## Plot losses 
plot_losses(OUTPUT_DIR, "cVAE CNN", train_losses, test_losses)
...
plot_image_collage(OUTPUT_DIR, "cVAE CNN", images, n_rows, n_cols)
...
X_gen = dec(z.to(device), C_gen.to(device))
X_gen = X_gen.detach().numpy()
X_gen = X_gen.reshape(len(X_gen), 28*28)

# Plot
plot_UMAP(X_gen, Y_gen, N_CLASSES, "cVAE CNN", OUTPUT_DIR)
...
torch.save(X_gen, os.path.join(OUTPUT_DIR, "cVAE_CNN_generated_images.pt"))